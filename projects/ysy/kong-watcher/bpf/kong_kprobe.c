...
static __always_inline void extract_http_path(const char *data, __u32 len, char *path) {
    ...
    // 두 번째 공백 찾기 (경로 끝)
    for (i = path_start; i < len && i < path_start + 63; i++) {
        if (data[i] == ' ') {
            path_end = i;
            break;
        }
...
static __always_inline void extract_http_header(const char *data, __u32 len, 
    ...
    for (i = 0; i < len - header_len - 2; i++) {
        ...
        if (j == header_len && data[i + j] == ':') {
            ...
            if (value_start < value_end && value_end - value_start < 31) {
...
int kprobe_sys_send(struct pt_regs *ctx) {
    // read와 유사한 로직으로 HTTP 요청 감지
    return kprobe_sys_read(ctx);
}

// recv 시스템 콜 kprobe
SEC("kprobe/__x64_sys_recv")
int kprobe_sys_recv(struct pt_regs *ctx) {
    // write와 유사한 로직으로 HTTP 응답 감지
    return kprobe_sys_write(ctx);
}
